...
export const API_PATH = '/api/generate-face'
export const VARIATIONS_API_PATH = '/api/generate-face-variations'
...
function parseRequestIds(url) {
  ...
  const raw = params.get('requestIds') ?? params.get('requestId') ?? ''
  return raw
    .split(',')
    .map((id) => id.trim())
    .filter(Boolean)
 ...
 * @param {import('node:http').IncomingMessage} req
 * @param {import('node:http').ServerResponse} res
 * @param {{ count?: number, body?: object }} [options]
 */
...
/**
 * Route fal.ai API requests (dev middleware + Vercel serverless).
 *
 * @param {import('node:http').IncomingMessage} req
 * @param {import('node:http').ServerResponse} res
 * @returns {Promise<boolean>} true if the request was handled
 */
export async function dispatchFalApi(req, res) {
